...
export default function LeaderboardClient({ 
  ...
  initialLeaderboard: LeaderboardRowPayload[]; 
  initialMovies: MoviePayload[]; 
}) {
  const [leaderboard, setLeaderboard] = useState<LeaderboardRowPayload[]>(initialLeaderboard);
  const [movies] = useState<MoviePayload[]>(initialMovies);
  const [personalTiers, setPersonalTiers] = useState<PersonalTierPayload[]>([]);
  const [loading, setLoading] = useState(false);

  // Selector inputs for personal placements
  const [selectedMovieId, setSelectedMovieId] = useState(initialMovies.length > 0 ? initialMovies[0].id : '');
  ...
  // Load personal placements (these are user-specific and must remain client-side)
  useEffect(() => {
