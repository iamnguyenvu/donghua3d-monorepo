...
export default function OngoingClient() {
  ...
  useEffect(() => {
    async function loadOngoingMovies() {
      try {
        setLoading(true);
        const res = await catalogApi.getMovies({ status: 'ongoing', sort: sortBy });
        if (res.success && res.data) {
          setMovies(res.data);
        }
      } catch (err) {
        console.error('Failed to load ongoing movies:', err);
      } finally {
        setLoading(false);
      }
    }
      ...
      <main className="w-full max-w-[1440px] mx-auto px-4 sm:px-6 md:px-12 lg:px-16 mt-28 flex-grow">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-zinc-500 bg-zinc-950/20 border border-zinc-900/50 rounded-[4px] my-12">
            Không tìm thấy bộ phim đang chiếu nào.
          </div>
        )}
      </main>
